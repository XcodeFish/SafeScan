 ...
 * @returns 代码位置范围
 */
function getNodeLocation(node: any, fileContent: string, filePath: string): CodeRange {
  // SWC节点都有span属性表示源码位置
  if (!node.span) {
    return {
 ...
 * @returns 节点对应的源代码
 */
function getNodeText(node: any, fileContent: string): string {
  if (!node.span) {
    return '';
  }
...
function createRuleContext(parseResult: TParseResult): RuleContext {
  ...
  return {
    ...
    fileContent,
    ast,
    getNodeLocation: (node: Node) => getNodeLocation(node, fileContent, filePath),
    getNodeText: (node: Node) => getNodeText(node, fileContent),
  };
}
 ...
 * @param visitor 访问回调
 */
function traverseAST(node: any, visitor: { enter: (node: any) => void }) {
  ...
  traverseAST(parseResult.ast, {
    enter(node: any) {
      ...
      for (const rule of rules) {
        try {
          const ruleIssues = rule.matcher(node, context);
